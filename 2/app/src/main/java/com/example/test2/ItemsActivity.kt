...
class ItemsActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Создаем ArrayList для хранения элементов
        val items = arrayListOf<Item>()
        // Получаем TextView, который ссылается на чат
        ...
        // Каждый элемент представляет собой различный продукт Arduino
        items.add(Item(1, "wifi", "Arduino с Wi-Fi модулем для IoT проектов", "Набор датчиков для IoT проектов на Arduino", "Arduino с Wi-Fi модулем ESP8266 позволяет разрабатывать различные IoT проекты, включая управление умными устройствами, сбор и передачу данных.", 1500))
        items.add(Item(2, "sensors", "Набор датчиков для Arduino", "Набор датчиков для IoT проектов на Arduino", "Набор датчиков для Arduino, включающий датчики температуры, влажности, освещенности и другие, позволяет создавать разнообразные IoT системы.", 1500))
        items.add(Item(3, "cloud", "Arduino с облачной поддержкой", "Arduino с поддержкой облачных сервисов для IoT", "Arduino с поддержкой облачных сервисов позволяет легко интегрировать ваши IoT проекты с облаком для удаленного управления и мониторинга.", 2500))
